...
if (test $ERRCODE != 0) then
  error "can't create $TEST_WORK_DIR/$UNIT/scafolddir subdir. error code=$ERRCODE"
fi
...
#
perenver=`echo $UNIT | awk -F "/" '{print $2}'`

if (test "$perenver" = "CPerennial9.1") then
   ln -s /path/to/TESTC++/C_Tests/PERENNIAL/peren.91/CVSA/SCAFFOLD/scaffold.c .
   ln -s $CTI_GROUPS/Lang/CPerennial9.1/CVSA/include .
else
   ln -s /path/to/TESTC++/C_Tests/PERENNIAL/peren_CTI/CVSA/SCAFFOLD/scaffold.c .
   ln -s $CTI_GROUPS/Lang/CPerennialLatest/CVSA/include .
fi

BCMD="$CC $CC_OPTIONS $DATA_MODE_FLAG ${CTI_OPT_SIGN}O${OPT_LEVEL} scaffold.c -c  -Iinclude"
...
if (test "$SHOW_SCRIPT_TRACE" = "true") then
  echo $ME: successfully built $TEST_WORK_DIR/$UNIT/scaffolddir/scaffold.o
fi
