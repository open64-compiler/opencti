...
Test case       : "#pragma omp parallel section lastprivate" construct verification.
...
int main(void) 
{
 ...
 #pragma omp parallel sections lastprivate(last) 
   {
     for (i=0; i < 10; i++) 
      {
       a[i] = b[i] * 2;
    ...
    { 
     if (last != 9 )
      printf ("The Test \"#pragma parallel sections lastprivate\" FAILED\n");
     else 
      printf ("The Test \"#pragma parallel sections lastprivate\" PASSED\n");
    }
   }
