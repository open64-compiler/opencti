...
int test_omp_sections_reduction(FILE * logFile){
	int sum=7;
	int known_sum;
	double dpt=1,dsum=0;
	double dknown_sum;
...
int main()
{
	...
	static const char * logFileName = "ctest_omp_sections_reduction.log";	/* name of the logfile */
	...
	fprintf(logFile,"##############################################\n");
	fprintf(logFile,"Testing omp sections reduction\n\n");

	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of test_omp_sections_reduction out of %d\n\n",i+1,REPETITIONS);
		if(test_omp_sections_reduction(logFile)){
